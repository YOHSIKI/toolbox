...
- **観測レッスン**: reserve API から取得した実スケジュールの行を `(店舗・ルーム・日付・時刻・program_id)` 単位で蓄積した記録。揺れた表記の補正と、月間テンプレートに無い日の仮スケジュール組み立ての両方で使う。
- **店舗マスター**: 店舗・ルームの識別子と表示名。UI プルダウンの選択肢。

保持期間は実行履歴が 90 日、予約結果が 365 日、スケジュールキャッシュが当月 + 前月（テンプレート）と直近 7 日（実スケジュール）を目安とする。観測レッスンは仮スケジュール用途のため過去 21 日分（3 週間）以上の保持が必要。
...
- **仮スケジュール**: カレンダー描画時に、実スケジュールも月間テンプレートも持たない日について観測レッスンから組み立てる。target 日 × 候補 3 週分の日付を 1 クエリでバッチ取得し、新しい候補日（−7 日）から順に観測ありの週を採用する。永続化は不要で、要求時に計算する。出典日は Lesson エンティティに `tentative_source` として持たせ、UI 側で「仮」バッジ表示と Intent 登録フォームの分岐に使う。
...
| 日付 | バージョン | 概要 |
|---|---|---|
| 2026-04-25 | 0.3 | 観測レッスンエンティティを明記。仮スケジュールを配置プレビュー・当日サマリーと並ぶ「導出データ」として追加。観測レッスンの保持期間を 21 日以上に変更（仮スケジュール用途）。 |
